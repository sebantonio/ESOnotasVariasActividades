...
- **Entornos de build (varias máquinas, no confundir)**:
  - PC "seban" (disco C: casi lleno): toolchain Rust en D:\rust (`RUSTUP_HOME=D:\rust\rustup`, `CARGO_HOME=D:\rust\cargo`). D: es NTFS (reformateado desde exFAT, que no soporta hardlinks — rustup los necesita)
  - PC "dell-seba" (C: con espacio de sobra, ~384GB libres): toolchain Rust por defecto en `%USERPROFILE%\.cargo` / `.rustup`, sin redirigir a D:. VS BuildTools 18 instalado; `link.exe`/`cl.exe` no están en el PATH normal — cargar antes de compilar: `& "C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\Common7\Tools\VsDevCmd.bat" -arch=x64`
    - **OJO**: en dell-seba `H:\Mi unidad\...` es un disco virtual montado por Google Drive File Stream (modo streaming, no carpeta local sincronizada) — matar `GoogleDriveFS.exe` desmonta el disco entero (hay que relanzar `C:\Program Files\Google\Drive File Stream\<ver>\GoogleDriveFS.exe` para recuperarlo). Además, con Drive activo, `npm install` corrompe ficheros de `node_modules` en curso (quedan a 0 bytes — mismo problema que la corrupción de `.git` del pendiente #1, ahora también en `node_modules`). Workaround: copiar el repo (sin `.git`/`node_modules`/`target`/`tauri-web`/`exe`) a una ruta local fuera de Drive (p.ej. `%TEMP%\...`), hacer `npm install` + `npx tauri build` ahí, y copiar el instalador resultante de vuelta a `exe/` en el repo real. `cargo`/`rustc` no tienen este problema porque `CARGO_TARGET_DIR` ya apunta fuera de Drive (`C:\cargo-target\plantillaNotas`)
